...
export function CnpjCpfValidator(tipoPessoa: TipoPessoa): AsyncValidatorFn {
  return (control: AbstractControl): Observable<ValidationErrors> => {
    ...
    if (tipoPessoa === TipoPessoa.PESSOA_FISICA) {
      valid = validaCPF(control.value);
    } else {
      valid = validaCNPJ(control.value);
    }

    return valid ? of(null) : of({ cnpjCpfInvalid: true });
  };
}
